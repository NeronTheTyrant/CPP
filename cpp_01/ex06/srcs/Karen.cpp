...
/*   Updated: 2022/01/21 03:41:44 by mlebard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Karen::complain(std::string level)
{
	...
	complaintLevel lvl = getComplaintLevel(level);
	if (lvl == WRONG)
	{
		std::cout << "Unkown complaint" << std::endl;
		return ;
	}
	complaint = Karen::_complaint[lvl];
	(this->*complaint)();
}

complaintLevel	Karen::getComplaintLevel(std::string level)
{
	complaintLevel	lvl;
	for (int i = 0; i < 4; i++)
	{
		if (level.compare(Karen::_complaintLevelString[i]) == 0)
		{
			lvl = complaintLevel(i);
			return (lvl);
		}
	}
	return (WRONG);
...
std::string	Karen::_complaintLevelString[4] = {
	"DEBUG",
	"INFO",
	"WARNING",
	"ERROR"
